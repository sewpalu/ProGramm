...
void CYKVisualiser::dumpContent()
{
	...
	{
		for (unsigned int colCount = 0; colCount < this->matrix.at(lineCount).size(); colCount++)
		{
			std::string displayPlace = "";
			//std::cout << "Line Count: " << lineCount << " | Column Count: " << colCount << "\n";
			for (unsigned int elementCount = 0; elementCount < this->matrix.at(lineCount).at(colCount).size(); elementCount++)
			{
				try {
					if (!(displayPlace.find(this->matrix.at(lineCount).at(colCount).at(elementCount).getRoot().getIdentifier()) != std::string::npos)) {
						if (displayPlace.size() > 0)
						{
							displayPlace = displayPlace + ", ";
						}
						displayPlace = displayPlace + this->matrix.at(lineCount).at(colCount).at(elementCount).getRoot().getIdentifier();
					}
				}
				catch (...) {
					std::cout << "Error when writing!\n";
					std::cout << "Matrix size i (lines): " << this->matrix.size() << "\n";
					std::cout << "Matrix size j (colunns): " << this->matrix.at(lineCount).size() << "\n";
					std::cout << "Matrix size k (CYKLinks in Cell): " << this->matrix.at(lineCount).at(colCount).size() << "\n";
					std::cout << "lineCount: " << lineCount << "colCount: " << colCount << "elementCount: " << elementCount << "\n";
				}
			}
			...
			for (unsigned int k = 0; k < this->matrix.at(i).at(j).size(); k++)
			{
				try {
					//if (!(displayPlace.find(this->matrix.at(i).at(j).at(k).getRoot().getIdentifier()) != std::string::npos)) {
						...
						displayPlace = displayPlace + this->matrix.at(i).at(j).at(k).getRoot().getIdentifier() + std::to_string(this->matrix.at(i).at(j).at(k).getProductions().size());
...
//Option to handle all recursive calls to further functions in order to return all possible SyntaxTrees that produce the word within the given grammar
std::vector<SyntaxTree> CYKVisualiser::convertToSyntaxTree(FormalGrammar grammar)
{
	...
	//Loop through all Nonterminals in the field of the CYKMatrix most upwards and leftwards
	for (unsigned int i = 0; i < this->matrix.at(wordLength - 1).at(0).size(); i++)
	{
		//Write all starting positions to vector
		if (this->matrix.at(wordLength - 1).at(0).at(i).getRoot().getIdentifier() == grammar.start.getIdentifier()) startPositions.push_back(i);
	}
	...
	for (unsigned int startPosCounter = 0; startPosCounter < startPositions.size(); startPosCounter++)
	{
		std::cout << "Hello from the loop ----------------------------- (iteration: " << startPosCounter << ") \n";
		CYKLink startPosition = this->matrix.at(wordLength - 1).at(0).at(startPositions.at(startPosCounter));
		std::cout << "Start position: " << startPosition.getRoot().getIdentifier() << "\n";
		STNode treeRoot(startPosition.getRoot().getIdentifier());
...
std::vector<STNode> CYKVisualiser::addChildrenFromLink(CYKLink inputLink)
{
	...
	{
		std::cout << inputLink.getProductions().at(i).second.getRoot().getIdentifier() << "   ";
	}
	std::cout << "\n";
	for (unsigned int childCounter = 0; childCounter < inputLink.getProductions().size(); childCounter++)
	{
		//std::cout << "Funktionsaufruf aus Schleife!\n";
		STNode child(inputLink.getProductions().at(childCounter).second.getRoot().getIdentifier());
		child.addChildren(addChildrenFromLink(inputLink.getProductions().at(childCounter).second));
	...
	//if (children.size() == 0) std::cout << "Letzter Wert: " << inputLink.getRoot().getIdentifier() << "\n\n";
